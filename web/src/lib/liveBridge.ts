 ...
 * Reconnect runs on a 5 s timer; logging is throttled so a missing backend
...
class LiveBridge {
  ...
  private handleMessage(raw: unknown): void {
    ...
    // `state` and `alert` are intentionally ignored here — the existing
    // simulator continues to drive Device.status / alerts so we don't double up.
  }
